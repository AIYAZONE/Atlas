...
- `type` 是组件的唯一匹配键，页面实例的 `SectionInstance.type` 会用它来定位对应的组件与 schema（文件名/组件文件名不作为匹配依据）。

- 需要“可重复子项/列表项”（例如轮播的多张图）时，不要在 settings 里自造列表字段类型；应使用 Shopify 兼容的 `blocks`（定义）+ `blocks + block_order`（实例）结构，详见 [07_Schema设计规范_Shopify兼容版.md](file:///Users/brucewang/Documents/AIYA/Atlas/.trae/documents/02_Technical_Architecture/07_Schema设计规范_Shopify兼容版.md) 与 [shopify-compatible.ts](file:///Users/brucewang/Documents/AIYA/Atlas/packages/schema/src/shopify-compatible.ts)。
