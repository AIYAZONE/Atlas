...
* **后端 API (MVP Phase 1-2)**:

  * Framework: Supabase (PostgreSQL + GoTrue + Realtime)
  * API: REST + GraphQL (Auto-generated)
  * Edge: Vercel Edge Functions (Custom Logic)

* **后端 API (Target Phase 3+)**:

  * Framework: Java Backend（Spring Boot 生态）
  * Data: PostgreSQL（JSONB，从 Supabase 迁移）
...
* CI/CD流程：GitLab CI/CD（自动化 Lint、Test、预览部署与生产发布）；详细规范见 [12_GitLab_CI_CD配置规范.md](../03_devops_risk/12_GitLab_CI_CD配置规范.md)
