...
### 发布审计记录口径（必须包含装配版本）
- 审计记录应能完整还原一次发布的“装配组合版本”：`site_id + theme_id/theme_version + feature_versions + content_version + config_version`。
- 回滚应基于上一条审计记录的“完整组合”执行，避免仅回退部分组件版本导致站点不一致（边界定义见 [07\_平台与站点工程边界（MVP方案A）.md](./07_平台与站点工程边界（MVP方案A）.md)）。
